...
public class CreatedExpiryTest extends CacheExpiryTest {
	...
	public static void setUp() {
		...
		config.setExpiryPolicyFactory(CreatedExpiryPolicy.factoryOf(EXPIRY));
	...
	public void t0002_get_expiry_update() {
		...
		sleep(FIRST_HALF_WAIT);
		assertTrue(cache.containsKey(key));
		cache.put(key, ualue); // the "update" action - update does NOT reset the clock
		sleep(SECOND_HALF_WAIT);
		assertNull(cache.get(key)); // this is the method we are testing for expiry
	}
	...
	public void t0003_get_expiry_access() {
		...
		sleep(FIRST_HALF_WAIT);
		assertTrue(cache.containsKey(key));
		assertEquals(value, cache.get(key)); // the "access" action - access does NOT reset the clock
		sleep(SECOND_HALF_WAIT);
		assertNull(cache.get(key)); // this is the method we are testing for expiry
	}
	...
	public void t0018_replace_expiry_update() {
		...
		cache.put(key, ualue); // the "update" action - update does NOT reset the clock
		sleep(SECOND_HALF_WAIT);
		assertFalse(cache.replace(key, walue));
		assertFalse(cache.containsKey(key));
	}
	...
	public void t0022_replaceIfSame_expiry_update() {
		...
		cache.put(key, ualue); // the "update" action - update does NOT reset the clock
		sleep(SECOND_HALF_WAIT);
		assertFalse(cache.replace(key, ualue, walue));
		assertFalse(cache.containsKey(key));
	}
